...
def ElAz_to_RAdec(el, az, lat, lon):
    ...
    RA1 = acos(-(sin(el)*cos(lat)*cos(lon) - cos(el)*cos(az) *
...
def ElAz_to_HAdec(el, az, lat):
    ...
    def f(h): return abs(-cos(el)*sin(az)-cos(dec)*sin(h))
...
def to_uv(lon, lat, X1, Y1, Z1, X2, Y2, Z2, el, az, freq, alg=0):
    ...
    X1, Y1, Z1, X2, Y2, Z2 = X1/omega, Y1 / \
        omega, Z1/omega, X2/omega, Y2/omega, Z2/omega
